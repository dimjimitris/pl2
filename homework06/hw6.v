...
Definition ie_io : term :=
  <[
    let rec ieio : (Nat -> Bool) * (Nat -> Bool) :=
    (
...
Module EnvInterp.
  ...
  Fixpoint eval (fuel : nat) (env : environment) (t : term) : option value :=
    ...
    | S fuel' =>
        ...
        (* Let *)
        | <[ let x := t1 in t2]> =>
          v1 <- eval fuel' env t1 ;; eval fuel' (add x v1 env) t2
        (* If *)
        | <[ if t1 then t2 else t3 ]> =>
        ...
        | <[ fix t ]> => 
          v <- eval fuel' env t ;;
          match v with
          | V_Clo env' x T t' => eval fuel' (add x (V_Fix env' x T t') env) t'
          (** Η ιδέα για τη περίπτωση του fix είναι ότι θέλουμε το t' να matchαρει
              με το T_Lambda στον επόμενο 'γύρο' και να αποθηκευτούν στο environment
              οι μεταβλητές που υπάρχουν τώρα στην εκτέλεση του προγράμματος και
              η μεταβλητή εισόδου της fix συνάρτησης (ώστε να πάρει την τιμή αυτή
              το πρώτο όρισμα κατά την εκτέλεση). Για τον λόγω αυτό επιστρέφουμε ένα
              περιβάλλον που είναι το αρχικό και έχει αυξηθεί με την αντίστοιχη μεταβλητή
              που θα πάρει την τιμή της fix.
          *)
          | _ => None
          end
        (* Values*)
        ...
        | T_Var x => 
          ...
          (** Στο lookup θέλουμε να 'διώξουμε' το fix αποτιμώντας το στο περιβάλλον του,
          αυξάνουμε το αντίστοιχο περιβάλλον με το όρισμα που περιέχει το fix ώστε να
          επιτρέψουμε την αναδρομική κλήση του fix operator. Με αυτό τον τρόπο 'σπρώχνουμε' τις
          εμφανίσεις του fix πιο μέσα στο 'δέντρο' της εκτέλεσης και έτσι μπορούμε να μην
          ασχοληθούμε με fix terms στον υπόλοιπο κώδικα (όπως π.χ. στο application, γιατί το fix
          έχει σπρωχτεί πιο μέσα και το application θα συναντήσει ένα V_Clo value)
          *)
          | val => return val
          end
        end
