...
Proof.
  ...
  induction m.
  - exists 0, 0. lia.
  - inv IHm. inv H0. inv H1.
    destruct (S x0 =? n) eqn: Heq.
    + apply Nat.eqb_eq in Heq.
      exists (S x), 0. split.
      2:{ assumption. }
      simpl. rewrite plus_n_Sm. rewrite Heq. lia.
...
Proof.
    ...
    assert (H4: (q1 - q2) * n < n). lia.
    assert (H5: (q2 - q1) * n < n). lia.
    ...
    - lia.
    - clear H4 H5.
      assert (H4: q1 = q2). lia. subst.
      clear H1 H3 Heq1 Heq2. lia.
Qed.
...
Module Nondet.
  ...
  | CAsgnChoiceList : string -> list aexp -> com.
  ...
  | E_AsgnChoiceL : forall st a1 a2 n1 x,
      ainterp st a1 = n1 ->
      st =[ x := a1 | a2 ]=> (x !-> n1 ; st)

  | E_AsgnChoiceR : forall st a1 a2 n2 x,
      ainterp st a2 = n2 ->
      st =[ x := a1 | a2 ]=> (x !-> n2 ; st)

  | E_AsgnChoiceAlt : forall st a1 a2 n x,
      (ainterp st a1 = n \/ ainterp st a2 = n) ->
      st =[ x := a1 | a2 ]=> (x !-> n ; st)
                               

  | E_AsgnChoiceLst : forall st aexps i a n x,
      nth_error aexps i = Some a ->
      ainterp st a = n ->
      st =[ CAsgnChoiceList x aexps ]=> (x !-> n ; st)
  ...
  Proof.
    ...
    - eapply E_AsgnChoiceL.
      simpl. reflexivity.

    - eapply E_AsgnChoiceR.
      simpl. reflexivity.
