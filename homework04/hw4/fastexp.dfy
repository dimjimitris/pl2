...
// Το λήμμα αυτό είναι επηρεασμένο από τον τρόπο που τροποποιούνται
// η βάση και ο εκθέτης στον αλγόριθμο της γρήγορης ύψωσης σε δύναμη
// καθώς και την μετατροπή του εκθέτη που χρειάστηκε να κάνουμε στη
// λύση της αντίστοιχης άσκησης στο coq. Είχαμε χρησιμοποιήσει την σχέση:
// exp = 2 * (exp / 2) + exp mod 2
ensures pow(x, n) == pow(x * x, n / 2) * pow(x, n % 2)
{
  if n == 0 {
