...
Theorem division_theorem_existence :
  forall (m n : nat),
    ...
    exists (q r : nat),
      m = q * n + r /\ r < n. 
Proof.
  intros m. induction m; intros n Hlt.
  - exists 0,0.
    split; lia.

  - destruct (IHm n Hlt) as [q [r [Hleq Hlt']]].

    destruct (r + 1 =? n) eqn:Heq.

    + apply Nat.eqb_eq in Heq; subst.
      exists (q + 1), 0. split; lia.
...
Theorem division_theorem_uniqueness :
  forall (m n q1 q2 r1 r2 : nat),
    ...
    q1 = q2 /\ r1 = r2.
Proof.
  intros m n q1 q2 r1 r2 Hlt Heq1 Hlt1 Heq2 Hlt2.
  subst. 
  
  assert (Hlt3 : (q1 - q2)*n < n) by lia.

  assert (Hlt4 : (q2 - q1)*n < n) by lia.

  destruct (q1 - q2) eqn:Heq; [ | lia (* impossible! *) ]. 
  destruct (q2 - q1) eqn:Heq'; [ | lia (* impossible! *) ].
  
  assert (Heq'': q1 = q2) by lia. subst.
...
Proof.
  ...
  eassumption. (* eassumption will unify [?y] with [11] *)
Qed.
...
Proof.
  ...
  Fail eassumption. (* whoops! [eassumption fails as [x] was not present in the context
...
Module Nondet.
  ...
  | E_WhileTrue : forall st st' st'' b c,
      ...
      st' =[ while b do c ]=> st'' ->
      st =[ while b do c ]=> st''
  | E_AsgnChoice_left : forall st a1 a2 n x,
      ainterp st a1 = n ->
      st =[ x := a1 | a2 ]=> (x !-> n ; st)
  | E_AsgnChoice_right : forall st a1 a2 n x,
      ainterp st a2 = n ->
      st =[ x := a1 | a2 ]=> (x !-> n ; st)
  ...
  Proof.
    ...
    - eapply E_Seq.
      ...
      apply E_AsgnChoice_left. reflexivity.
      
    - eapply E_Seq.
      eapply E_Asgn. reflexivity.
      
      apply E_AsgnChoice_right. reflexivity.
      
    - exists Z. discriminate. 
  Qed.
